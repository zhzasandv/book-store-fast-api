...
```
frontend/
├── app.vue              # Корневой компонент — просто <NuxtPage />
├── nuxt.config.ts       # Конфигурация: apiBase = http://localhost:8000
├── pages/
│   └── index.vue        # Вся страница каталога
├── composables/
│   └── useBooks.ts      # Логика: fetch + поиск
└── types/
...
### `composables/useBooks.ts` — логика загрузки

```typescript
export function useBooks() {
  const config = useRuntimeConfig()
  const searchQuery = ref('')  // реактивная строка поиска
  ...
  const { data, pending, error } = useFetch<Book[]>(
    ...
    { watch: [searchQuery] }  // перезапрашивать при изменении searchQuery
  )
...
- `watch: [searchQuery]` — автоматически повторяет запрос при изменении поля поиска
- URL меняется динамически: если есть запрос → `/search?q=...`, иначе → `/books/`

### `pages/index.vue` — главная страница

Компонент использует composable `useBooks` и отрисовывает:
- Заголовок
- Поле поиска (v-model → searchQuery)
- Состояния: загрузка / ошибка / пусто / сетка книг
- Карточки книг с обложкой, названием, авторами, жанром, ценой

```vue
<input v-model="searchQuery" placeholder="Поиск по названию, автору, жанру..." />
```

Тёмная тема с Indigo-акцентами.
